...
{ "os": "$os",
  "arch": "$arch"}
EOF
    jq --slurpfile arch $tmp_dir/${os}_${arch}.json '.versions[0].platforms += $arch' $tmp_dir/provider_current.json >> $tmp_dir/provider_current_updated.json
...
EOF
  ...
  if [[ -f pages/_data/providers.json ]]; then
    jq -s --arg var "$NAMESPACE/$name" 'map(select(.id == $var))| {id: .[0].id, versions: map( .versions ) | add}' pages/_data/providers.json $tmp_dir/provider_current.json
  else
